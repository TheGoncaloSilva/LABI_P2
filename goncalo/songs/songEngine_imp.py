...
# @return -> duração música 
def durationData(data):
    return round(data.getnframes() / float(data.getframerate()))
...
def effectsSong(data, bpm, effects):
    if effects == "fadein" : # efeito de Fade In
        ...
        data = normalizeSong(data)
    elif effects == "modular" : # Aplicar uma modulação (multiplicar um som por outro)
        data = modulateSong(data, bpm, 441) # choose new FREQ !!!!!!
    elif effects == "delay" : # Introduzir um delay à música
        data = delaySong(data, bpm, 5, 2) # choose new values !!!!!!!!
...
def createSong(dictionary):
    ...
    for i, music in enumerate(dictionary["music"]) :
        if len(music) == 1 :
            ...
            dataTemp = [w.getparams(), w.readframes(w.getnframes())]
            if i in dictionary["effects"]:
                dataTemp = effectsSong(dataTemp, calculateFramerate(dictionary["bpm"]), dictionary["effects"][i])
            ...
            for l, pos in enumerate(music) :
                ...
                dataTemp = [w.getparams(), w.readframes(w.getnframes())]
                if i in dictionary["effects"]:
                    overlay += effectsSong(dataTemp, calculateFramerate(dictionary["bpm"]), dictionary["effects"][i])
                else :
                    overlay += dataTemp
            data.append(overlay)
        else :
    ...
    if dictionary["mask"] != "none" : 
        data = maskSong(data, calculateFramerate(dictionary["bpm"]), dictionary["mask"], 0, durationData(data))
