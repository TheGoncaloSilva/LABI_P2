...
def durationData(data, framerate): # aceita bytes como parâmetros, invés de um filepath (Não está a funcionar muito corretamente)
    return round(data / float(framerate)) # devolver a duração da música em segundos
...
def fadeInSong(song, sample_rate, duration) : 
    ...
    for index, value in enumerate(song):
        ...
        if time > time_start and time < time_stop :
            new_song.append(value * index * int(step)) # usar o int para não dar erro de conversão com os outos valores
        else :
...
def modulateSong(song, sample_rate, freq) : # Não está a funcionar corretamente
    ...
    freq = int(freq)
    for index, value in enumerate(song):
        new_song.append(value * math.sin(2 * math.pi * freq * index / int(sample_rate))) # adicionei o int, pois estava a ter erros de operação com outras variáveis
...
def createSong(dictionary):
    ...
    for i, music in enumerate(dictionary["music"]) :# Percorrer o dicionário das músicas
        if len(music) == 1 : # se não for preciso combinar músicas, apenas se acrescenta ao dicionário
            ...
            for l, pos in enumerate(music) : # percorrer as músicas presentes no índice
